...
module.exports = function getWithModulesFromParent (modulesFromParent) {
  // To make sure that we use the same version of React on the client and
  // server, these are passed in by the consuming developer
  ...
  Ambidex.prototype._verifyPaths = function () {
    ...
    return Promise.all(
      ...
      () => {
              ...
              // Make sure we have our own instance of <AmbidexRoutes> and its `handlerProps` has our `settings`
              var routes = require(self._get("routesPath"));
